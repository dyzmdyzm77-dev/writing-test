...
async function popupRecommendFlow(model: string | undefined): Promise<boolean> {
  ...
  for (const el of elements) {
    try { const s = await refineAiSuggestions(await fetchAiSuggestions(el.text, model)); results.push({ role: el.role, original: el.text, suggestions: s }); }
    catch (e) { results.push({ role: el.role, original: el.text, suggestions: [], error: errStr(e) }); }
  }
