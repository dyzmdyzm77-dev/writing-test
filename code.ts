...
const TERM_RULES: FixRule[] = [
  ...
  { pattern: /미동의(?!자)/g, replacement: "동의 안함", reason: "표준 용어로 통일해요.", tags: ["term"] },
  { pattern: /미표시(?!자)/g, replacement: "표시 안함", reason: "표준 용어로 통일해요.", tags: ["term"] },
  { pattern: /미등록(?!자)/g, replacement: "등록 안됨", reason: "표준 용어로 통일해요.", tags: ["term"] },
  // "출입 가능성/출입 불가능"의 일부를 잘라먹지 않도록 가드
  { pattern: /출입 가능(?!성)/g, replacement: "출입 허용", reason: "표준 용어로 통일해요.", tags: ["term"] },
...
// ===============================
const COMPOUND_PROTECT_RULES: FixRule[] = [
...
function alignWhitespace(original: string, corrected: string): string {
  ...
  if (oLines.length !== cLines.length) return original;
  const out: string[] = [];
...
// ===============================
const ACTION_NOUNS =
  '확인|문의|저장|삭제|등록|입력|선택|설정|변경|수정|추가|취소|신청|동의|인증|연결|해제|시도|사용|적용|이동|클릭|터치|검색|조회|작성|제출|첨부|업로드|다운로드|로그인|로그아웃|재시작|시작|종료|갱신|예약|결제|가입|인쇄|출력|복사|백업|복원|차단|허용';
