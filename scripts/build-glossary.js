...
const psScript = [
  ...
  "  if (-not $silent) {",
  "    $r = [System.Windows.Forms.MessageBox]::Show(\"설치는 끝났어요. 그런데 Node.js가 없어요.`n`n[확인]을 누르면 다운로드 페이지가 열립니다.`nNode.js 설치를 마친 뒤 이 파일을 다시 실행해 주세요.\", '클로드 커넥터 설치 (1/2) — Node.js', 'OKCancel', 'Warning')",
  "    if ($r -eq 'OK') { Start-Process 'https://nodejs.org/ko/download' }",
  "  }",
  "  exit",
  "}",
  "if (-not (Get-Command claude -ErrorAction SilentlyContinue)) {",
  "  Box \"설치는 끝났어요. 그런데 Claude Code가 없어요 (또는 PATH에 없어요).`n`n터미널에서 아래를 설치·로그인한 뒤 이 파일을 다시 실행해 주세요:`n`n  npm install -g @anthropic-ai/claude-code`n  claude login`n`n확인: 터미널에서 claude --version 이 버전을 출력하면 준비 완료.`n(사용량은 이 PC에 로그인된 클로드 구독 한도에서 차감됩니다.)\" '클로드 커넥터 설치 (2/2) — Claude Code' 'Warning'",
  "  exit",
  "}",
  "Start-Process -FilePath 'cmd.exe' -ArgumentList '/c node scripts\\claude-bridge.js' -WorkingDirectory $dir -WindowStyle Hidden",
  "Box \"설치 완료! 클로드 커넥터를 연결했어요.`n`n이제 피그마 플러그인으로 돌아가 [추천받기]를 누르면 클로드가 답해요.`n다음부터는 플러그인에서 추천·번역 화면에 들어가면 자동으로 연결됩니다.\" '클로드 커넥터 — 준비 완료' 'Information'",
].join('\n');
