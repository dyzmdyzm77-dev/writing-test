...
- **추천 AI 우선순위**: 예시 사전(완전/포함/유사 매칭 — 즉답) → 클로드 다리(로컬 브리지, 켜져 있으면) → Gemini(개인 키 → 서버 공용 키 GEMINI_API_KEY 환경변수) → 유사 예시+규칙 폴백. 사전 히트 시에도 AI 가능하면 자동으로 AI 카드를 이어 붙임. 추천 화면 헤더의 [AI 추천] 토글(clientStorage 'aiRecommendOff')을 끄면 로컬만 사용. 번역은 개인 키 → 서버 공용 키 순(로컬 대체 없음). 서버가 noKey를 회신하면 세션 동안 재시도 안 함(serverSharedKeyMissing). 예시 사전의 추천안은 검토 규칙과 모순되면 안 됨(마침표·'해 주세요' 띄어쓰기 포함) — vm 샌드박스로 suggestFriendlyKorean(추천안)이 무변경인지 확인
- **클로드 다리(scripts/claude-bridge.js)**: `claude -p` stream-json 세션을 상시 유지하는 로컬 서버(localhost:11888). 켜기: 클로드다리-켜기.bat / 자동시작등록.vbs, 끄기: 클로드다리-끄기.bat(POST /shutdown). 속도 핵심 3가지 — 세션 재사용(시동 1회), **빈 폴더 cwd**(저장소에서 실행하면 프로젝트 맥락 로딩으로 45초/턴), MAX_THINKING_TOKENS=0 + CLAUDE_CODE_DISABLE_NONESSENTIAL_TRAFFIC=1 (→ 약 3초/턴). 같은 문구 재요청이면 "이전과 다른 제안" 지시(askedCount). 기본 모델 sonnet, 플러그인 드롭다운(modelSelect)이 요청마다 model을 실어 보냄 — 다리는 모델이 바뀌면 세션 재시작(재워밍업 ~18초, 같은 모델은 5~10초). startProc의 close 핸들러는 `proc !== thisProc`면 무시(모델 전환 시 옛 세션 close가 새 세션을 죽이는 것 방지). manifest는 **devAllowedDomains**에 localhost 등록 — allowedDomains에 http를 넣으면 manifest 검증 실패로 플러그인이 목록에서 사라짐
