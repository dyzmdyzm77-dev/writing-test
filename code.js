...
async function fetchAiSuggestions(text, apiKey, bridge) {
    ...
    try {
        ...
        if (e instanceof Error && (e.message.indexOf('AI 추천 실패') >= 0 || e.message.indexOf('클로드 추천 실패') >= 0))
            throw e;
    ...
    if (msg.type === "RECOMMEND") {
        ...
        try {
            ...
            if (msg.forceAi)
                ...
                postRecommendFallback(text, ''); // 공용 키 미등록 → 키 안내 폴백 (재시도 버튼 없이)
            else
                postRecommendFallback(text, errStr(e), undefined, true); // AI 사용 가능 상태였으니 재시도 버튼 노출
        }
        return;
    }
