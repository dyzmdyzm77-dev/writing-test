...
const RECOMMEND_EXAMPLES = [
    ...
    { input: "본 계약의 유일한 마스터 관리자로 일반관리자로 권한변경을 하실 수 없어요. 일반 관리자로 권한 변경을 원하실 경우 다른 사람에게 마스터 관리자 권한을 지정해 주신 후 다시 시도해 주세요.", suggestions: ["다른 사람을 마스터 관리자로 지정한 뒤 일반 관리자로 변경할 수 있어요.", "다른 사람을 마스터 관리자로 지정하면 변경할 수 있어요.", "유일한 마스터 관리자는 일반 관리자로 변경할 수 없어요.\n다른 사람을 마스터 관리자로 지정하면 변경할 수 있어요."] },
];
// ===== RECOMMEND:END =====
...
function postRecommendFallback(text, failNote, emptyNote) {
...
async function isAiRecommendOn() {
    try {
...
async function readUsage() {
    ...
    let stored = null;
    try {
        stored = await figma.clientStorage.getAsync(USAGE_STORAGE);
    }
    catch (_e) { /* 무시 */ }
...
async function bumpUsage() {
    ...
    try {
        await figma.clientStorage.setAsync(USAGE_STORAGE, next);
    }
    catch (_e) { /* 무시 */ }
    return next;
}
function postUsage(u) {
    figma.ui.postMessage({
        type: 'usage-update',
        used: u.count,
...
figma.ui.onmessage = async (msg) => {
        ...
        try {
            ...
            if (!res.ok || data.error || !data.suggestions || !data.suggestions.length) {
                ...
                if (msg.forceAi)
                    ...
                    postRecommendFallback(text, failNote);
                return;
            }
            ...
            if (msg.forceAi)
                ...
                postRecommendFallback(text, 'AI 추천 실패: ' + errStr(e));
        }
        return;
    }
    ...
    if (msg.type === "SET_API_KEY") {
        ...
        try {
            await figma.clientStorage.setAsync(API_KEY_STORAGE, k);
        }
        catch (_e) { /* 무시 */ }
    ...
    if (msg.type === "SET_AI_TOGGLE") {
        try {
            await figma.clientStorage.setAsync(AI_TOGGLE_STORAGE, !msg.on);
        }
        catch (_e) { /* 무시 */ }
